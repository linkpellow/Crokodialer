...
class AudioManager {
  constructor() {
    ...
    // Sound file mappings aligned with Telnyx events
    this.soundMap = {
      'call.initiated': 'call-initiated.wav',
      'call.answered': 'call-answered.wav',
      'call.hangup': 'call-hangup.wav',
      'call.incoming': 'call-inbound.wav',
      'call.inbound': 'call-inbound.wav',
      'error': 'error.wav',
      'ui.button': 'ui-buttons.wav',
      'dtmf.received': 'ui-buttons.wav',
      'call.speak.started': 'ui-buttons.wav',
      'call.speak.ended': null, // No sound for speak ended
      'call.recording.saved': 'ui-buttons.wav'
    };
  ...
  async preloadSounds() {
    ...
    const soundFiles = Object.values(this.soundMap).filter((v, i, a) => v && a.indexOf(v) === i);
    ...
    for (const file of soundFiles) {
      try {
        ...
        console.log(`✅ [AudioManager] Preloaded: ${file}`);
      } catch (error) {
        console.error(`❌ [AudioManager] Failed to preload ${file}:`, error);
      }
    }
  }
  ...
  async playSound(eventType, options = {}) {
    ...
    const soundFile = this.soundMap[eventType];
    if (!soundFile) {
      console.log(`🔇 [AudioManager] No sound mapped for event: ${eventType}`);
      return;
    }
    
    // Add to queue if already playing (prevents overlap)
    if (this.isPlaying && !options.immediate) {
      this.soundQueue.push({ eventType, options });
      return;
    }
    
    try {
      console.log(`🔊 [AudioManager] Playing ${soundFile} for event: ${eventType}`);
      ...
      // Clone the preloaded audio for simultaneous plays
      ...
      console.error(`❌ [AudioManager] Error playing ${soundFile}:`, error);
      this.isPlaying = false;
      this.processQueue();
    }
  }
  ...
  processQueue() {
    if (this.soundQueue.length > 0) {
      ...
      this.playSound(next.eventType, next.options);
    }
  }
  ...
  setVolume(level) {
    ...
    console.log(`🔊 [AudioManager] Volume set to ${this.volume * 100}%`);
  }
...
dialButtons.forEach(button => {
  button.addEventListener('click', () => {
    audioManager.playSound('ui.button');
...
function connectWebSocket() {
  try {
    ...
    const wsUrlWithAuth = storedToken ? `${wsUrl}?token=${storedToken}` : wsUrl;
...
function handleWebSocketMessage(message) {
  ...
  switch (message.event) {
    ...
    case 'call-initiated':
      ...
      audioManager.playSound('call.initiated');
...
function handleLeadSelection(leadData) {
  ...
  console.log('[SOCKET INCOMING]', JSON.stringify(leadData));
  ...
  // Add lead to allLeads array if not already present
  const existingIndex = allLeads.findIndex(lead => lead.id === selectedLead.id);
  if (existingIndex === -1) {
    allLeads.push(selectedLead);
    console.log('✅ [WEBSOCKET] Added lead to allLeads array. Total leads:', allLeads.length);
  } else {
    allLeads[existingIndex] = selectedLead;
    console.log('✅ [WEBSOCKET] Updated existing lead in allLeads array');
  }
...
async function handleInitiateCall(callData) {
  ...
  console.log('[SOCKET INCOMING]', JSON.stringify(callData));
  
  try {
    // Extract call data
    const { leadId, phone, userId, name, mode } = callData;
    
    // Check for power dialer mode
    if (mode === 'power') {
      console.log('⚡ [POWER DIALER] Power dialer mode activated!');
      powerDialerEnabled = true;
    } else {
      console.log('📞 [DIALER] Regular dialing mode');
      powerDialerEnabled = false;
    }
    
    console.log(`📞 [WEBSOCKET] Starting outbound call to ${phone} for lead ${name} (mode: ${mode || 'regular'})`);
...
function playSFX(sfxFile) {
  // Map file names to event types for AudioManager
  const fileToEvent = {
    'call-initiated.wav': 'call.initiated',
    'call-answered.wav': 'call.answered',
    'call-hangup.wav': 'call.hangup',
    'call-inbound.wav': 'call.inbound',
    'error.wav': 'error',
    'ui-buttons.wav': 'ui.button'
  };
  
  const eventType = fileToEvent[sfxFile] || 'error';
...
function initializeAudioContext() {
  try {
    ...
    console.log('✅ [AUDIO] Audio context initialized for level monitoring');
  } catch (error) {
...
async function makeRealTelnyxCall(phoneNumber) {
  try {
    ...
    let attempts = 0;
    while (!window.TelnyxRTC && attempts < 10) {
      console.log(`⏳ [REAL CALL] Waiting for TelnyxRTC to load... attempt ${attempts + 1}`);
      await new Promise(resolve => setTimeout(resolve, 200));
      attempts++;
    }
    
    // Check if TelnyxRTC is available (should be loaded via preload.js)
    if (!window.TelnyxRTC) {
      console.error('❌ [REAL CALL] TelnyxRTC is not available!');
      console.error('Make sure preload.js is properly loading the SDK');
    ...
    console.log('🔐 [REAL CALL] Fetching Telnyx JWT...');
    const jwtResponse = await apiRequest('GET', '/auth/telnyx-jwt');
    const telnyxJwt = jwtResponse.jwt;
    ...
    // Try multiple event variations as different SDK versions use different names
    
    // Primary state event handler (as per official Telnyx docs)
    call.on('state', (state) => {
      console.log('📞 [REAL CALL] State event received:', state);
      console.log('📞 [REAL CALL] State type:', typeof state);
      console.log('📞 [REAL CALL] State value exactly:', JSON.stringify(state));
      
      // Check specifically for answered state
      if (state === 'answered') {
        console.log('🎉🎉🎉 [REAL CALL] ANSWERED STATE DETECTED!');
        setCallState(CALL_STATES.ACTIVE, `Connected to ${phoneNumber}`);
        audioManager.playSound('call.answered');
      } else {
        handleCallStateUpdate(state, phoneNumber);
      }
    });
    
    // Alternative: stateChange event
    call.on('stateChange', (state) => {
      console.log('📞 [REAL CALL] StateChange event:', state);
      handleCallStateUpdate(state, phoneNumber);
    });
    
    // Alternative: specific events
    call.on('connecting', () => {
      console.log('📞 [REAL CALL] Connecting event fired');
      setCallState(CALL_STATES.DIALING, `Calling ${phoneNumber}`);
      audioManager.playSound('call.initiated');
    });
    
    call.on('ringing', () => {
      console.log('📞 [REAL CALL] Ringing event fired');
      setCallState(CALL_STATES.RINGING, `Ringing ${phoneNumber}`);
    });
    
    call.on('accepted', () => {
      console.log('🎉 [REAL CALL] ACCEPTED event fired - CALL ANSWERED!');
      setCallState(CALL_STATES.ACTIVE, `Connected to ${phoneNumber}`);
      audioManager.playSound('call.answered');
      console.log('🎉 [REAL CALL] YOU SHOULD HEAR THE PERSON NOW!');
    });
    
    call.on('answered', () => {
      console.log('🎉 [REAL CALL] ANSWERED event fired - CALL ANSWERED!');
      setCallState(CALL_STATES.ACTIVE, `Connected to ${phoneNumber}`);
      audioManager.playSound('call.answered');
      console.log('🎉 [REAL CALL] YOU SHOULD HEAR THE PERSON NOW!');
    });
    ...
    call.on('ended', () => {
      console.log('📞 [REAL CALL] Ended event fired');
      setCallState(CALL_STATES.DISCONNECTED, 'Call ended');
      audioManager.playSound('call.hangup');
    });
    
    call.on('failed', (error) => {
      console.log('❌ [REAL CALL] Failed event fired:', error);
      setCallState(CALL_STATES.DISCONNECTED, 'Call failed');
      audioManager.playSound('error');
    });
    
    // Helper function to handle state updates
    function handleCallStateUpdate(state, phoneNumber) {
      switch (state) {
        case 'new':
        case 'connecting':
          setCallState(CALL_STATES.DIALING, `Calling ${phoneNumber}`);
        ...
        case 'failed':
          ...
          break;
        default:
          console.log('📞 [REAL CALL] Unknown state:', state);
      }
    }
              ...
              call.peerConnection.getStats().then(stats => {
                stats.forEach(report => {
                  if (report.type === 'inbound-rtp' && report.kind === 'audio') {
                    console.log('📊 [AUDIO STATS] Inbound:', {
                  ...
                  if (report.type === 'outbound-rtp' && report.kind === 'audio') {
                    console.log('📊 [AUDIO STATS] Outbound:', {
    ...
    if (call.peerConnection) {
      call.peerConnection.onsignalingstatechange = () => {
        ...
        // When signaling is stable after offer/answer exchange, call is connected
        if (signalingState === 'stable' && isCallActive) {
          console.log('🎉 [REAL CALL] Signaling stable - call connected!');
    ...
    // Debug: Log ALL events from the call object
    const originalEmit = call.emit;
    call.emit = function(event, ...args) {
      console.log('🔍 [CALL DEBUG] Event emitted:', event, 'Args:', args);
