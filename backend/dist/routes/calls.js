...
const telnyx_1 = require("../services/telnyx");
const outboundNumbers_json_1 = __importDefault(require("../../config/outboundNumbers.json"));
...
function findNearestAreaCode(targetAreaCode, options = {}) {
    const targetRegion = areaCodeRegions[targetAreaCode];
    if (!targetRegion) {
        console.warn(`⚠️ Unknown area code: ${targetAreaCode} - using random selection`);
        return {
            nearestNumber: outboundNumbers_json_1.default[Math.floor(Math.random() * outboundNumbers_json_1.default.length)],
            allDistances: []
        };
    }
    ...
    for (const number of outboundNumbers_json_1.default) {
        const numberAreaCode = number.slice(2, 5);
        const numberRegion = areaCodeRegions[numberAreaCode];
        if (numberRegion) {
            const distance = calculateDistance(targetRegion.coordinates[0], targetRegion.coordinates[1], numberRegion.coordinates[0], numberRegion.coordinates[1]);
...
function pickBestFrom(to, options = {}) {
    const toArea = to.slice(2, 5); // Extract area code
    console.log(`📞 Processing call to: ${to} (Area code: ${toArea})`);
    // First, try exact match
    const exactMatch = outboundNumbers_json_1.default.find(n => n.slice(2, 5) === toArea);
    if (exactMatch) {
    ...
    if (targetRegion && nearestRegion) {
        const distance = calculateDistance(targetRegion.coordinates[0], targetRegion.coordinates[1], nearestRegion.coordinates[0], nearestRegion.coordinates[1]);
...
router.post('/', async (req, res) => {
    try {
        ...
        try {
            const { call_control_id } = await (0, telnyx_1.startOutboundCall)({ fromNumber, toNumber: formattedToNumber });
